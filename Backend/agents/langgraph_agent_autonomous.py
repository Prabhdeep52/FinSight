...
class OptimizedAutonomousAgent:
    ...
    def __init__(self):
        ...
        logger.info(
            "OptimizedAutonomousAgent: Initializing - Instance ID: {uuid.uuid4()}"
        )
        ...
        self.tool_execution_map = {
            ...
            "get_news": self.search_tool._run,
        }
    ...
    def _create_structured_tools(self) -> List[StructuredTool]:
        ...
        class StockNameInput(BaseModel):
            name: str = Field(description="Stock company name (e.g., Apple, Microsoft)")
        ...
        tools = [
            ...
            StructuredTool.from_function(
                ...
                name="get_news",
                description="Get recent news articles about the company or anything else. Essential for understanding current events.",
                args_schema=StockSymbolInput,
            ),
        ]
    ...
    def process_query(
        self, user_query: str, session_id: str = None, user_id: str = "default_user"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if context:
                logger.info(
                    f"OptimizedAutonomousAgent: Using conversation context ({len(context)} chars)"
                )
            ...
            tool_plan, plan_reasoning = self._autonomous_planning_phase(
                user_query, context
            )

            if not tool_plan:
                logger.warning(
                    "OptimizedAutonomousAgent: No tools requested, generating direct response"
                )
                ...
                self.memory.save_message(
                    session_id, "assistant", state["final_response"], metadata={}
                )
            ...
            logger.info(
                f"OptimizedAutonomousAgent: Identified symbols: {state['extracted_symbols']}"
            )

            # Phase 2: Batch Execution (0 LLM calls - just execute)
            logger.info(
                f"OptimizedAutonomousAgent: PHASE 2 - Executing {len(tool_plan)} tools in parallel"
            )
            ...
            if additional_needed["needed"] and additional_needed.get("tools"):
                logger.info(
                    f"OptimizedAutonomousAgent: Fetching additional: {additional_needed['tools']}"
                )
                additional_plan = self._plan_additional_tools(
                    state, additional_needed["tools"]
                )
                if additional_plan:
            ...
            self.memory.save_message(
                session_id, "assistant", state["final_response"], assistant_metadata
            )
    ...
    def _autonomous_planning_phase(
...
- get_news(text) - Recent news articles or anything else about the company
...
- **you can search anything about the company using the search tool including latest news. mainly find news related to the company.
...
Remember: Explain first, then call all tools NOW!"""
        ...
        try:
            ...
            reasoning = (
                response.content
                if response.content
                else "Planning data collection strategy."
            )

            if tool_calls:
                logger.info(
                    f"OptimizedAutonomousAgent: LLM autonomously requested {len(tool_calls)} tools"
                )
                logger.info(
                    f"OptimizedAutonomousAgent: LLM reasoning: {reasoning[:100]}..."
                )
            else:
                logger.warning(
                    "OptimizedAutonomousAgent: LLM did not request any tools"
                )
    ...
    def _get_tool_purpose(self, tool_name: str) -> str:
        ...
        purposes = {
            ...
            "get_news": "recent news articles or anything else about the company",
        }
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.info(
                        f"OptimizedAutonomousAgent: Completed {tool_name}({symbol})"
                    )

                except Exception as e:
                    logger.error(
                        f"OptimizedAutonomousAgent: Error {tool_name}({symbol}): {e}"
                    )
    ...
    def _store_tool_result(self, state: Dict, tool_name: str, symbol: str, result: str):
        ...
        try:
            ...
            if isinstance(result_data, list):
                # Directly store for list-based tools (like get_news)
                if tool_name == "get_news":
                    state["news_data"][symbol] = result_data
                    print(result_data)
                    return
                else:
                    # Other tools shouldn't normally return lists
                    logger.warning(f"Unexpected list result for {tool_name}({symbol})")
                    return
            ...
            if tool_name == "get_stock_overview":
                ...
                # Redundant safety (if news tool later changes format)
                state["news_data"][symbol] = result_data
            else:
    ...
    **Question:** Is this SUFFICIENT to answer the query comprehensively?
    ...
    "tools": ["tool_name"],  // ONLY from the 6 tools above, ONLY if needed=true
    ...
    Be honest - only request more if truly necessary from available tools."""
        ...
        try:
            ...
            if json_match:
                ...
                valid_tools = [
                    t for t in requested_tools if t in self.tool_execution_map
                ]
                ...
                logger.info(
                    f"Review: needed={review.get('needed', False)}, confidence={review.get('confidence', 'unknown')}"
                )
                return review
            ...
            for tool_name in tool_names:
                ...
                if tool_name == "get_stock_overview":
                    if symbol in state["stock_data"]:
                        logger.info(
                            f"Already have stock_overview for {symbol}, skipping"
                        )
                        continue
                else:
                    data_type = tool_name.replace("get_", "")
                    if (
                        symbol in state["statement_data"]
                        and data_type in state["statement_data"][symbol]
                    ):
    ...
    **Structure:**
    ...
    def _create_context(self, state: Dict) -> str:
        ...
        for symbol in symbols:
            ...
            if stock_info:
                ...
                context_parts.append(
                    f"Current Price: ${stock_info.get('price', 'N/A')}"
                )
                context_parts.append(f"P/E Ratio: {stock_info.get('pe_ratio', 'N/A')}")
                context_parts.append(
                    f"52W Range: ${stock_info.get('week_52_low', 'N/A')} - ${stock_info.get('week_52_high', 'N/A')}"
                )
            ...
            if "income_statement" in statement_info:
                ...
                if "annualReports" in income_data and income_data["annualReports"]:
                    ...
                    context_parts.append(
                        f"Gross Profit Margin: {latest.get('grossProfitRatio', 'N/A')}"
                    )
            ...
            if "balance_sheet" in statement_info:
                ...
                if "annualReports" in balance_data and balance_data["annualReports"]:
                    ...
                    context_parts.append(
                        f"Total Debt: ${float(latest.get('totalDebt', 0)) / 1e9:.1f}B"
                    )
                    context_parts.append(
            ...
            if "cash_flow" in statement_info:
                ...
                if "annualReports" in cashflow_data and cashflow_data["annualReports"]:
                    ...
                    context_parts.append(
                        f"Operating Cash Flow: ${float(latest.get('operatingCashflow', 0)) / 1e9:.1f}B"
                    )
                    context_parts.append(
                        f"Free Cash Flow: ${float(latest.get('freeCashFlow', 0)) / 1e9:.1f}B"
                    )
            ...
            if "earnings" in statement_info:
                ...
                if (
                    ...
                    context_parts.append(
                        f"Latest EPS: ${latest.get('reportedEPS', 'N/A')}"
                    )
                    context_parts.append(
    ...
    def stream_execution(
        self, user_query: str, session_id: str = Any, user_id: str = "default_user"
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"stream_execution: Starting with session_id={session_id}, user_id={user_id}"
            )
            session_data = self.memory.get_or_create_session(session_id, user_id)
            ...
            context = self.memory.build_short_term_context(session_id)
            logger.info("stream_execution: Context built successfully")
            ...
            for tool_call in tool_plan:
                ...
                if self._is_data_in_session_cache(session_id, symbol, tool_name):
                    ...
                    # Use cached data
                    cached_data = self._get_from_session_cache(
                        session_id, symbol, tool_name
                    )
                    self._store_tool_result(
                        state, tool_name, symbol, json.dumps(cached_data)
                    )
                    data_type = tool_name.replace("get_", "").replace("_", " ")
                    state["fetch_summary"].append(
                        f"{data_type} for {symbol} (from session cache)"
                    )
            ...
            if cached_tools:
                ...
                yield {
                    "type": "step",
                    "step": "cache_hit",
                    "message": "Using cached data from earlier in this conversation",
            ...
            if not tools_to_execute:
                yield {
                    ...
                    "reasoning": "I already have all the data I need from our earlier exchange. No new API calls needed - proceeding directly to analysis!",
                    "progress": 70,
                }
            else:
                yield {
                            ...
                            try:
                                ...
                                if not (
                                    ...
                                    self._add_to_session_cache(
                                        session_id, symbol, tool_name, result_data
                                    )

                                    logger.info(
                                        f"Session cache STORED: {tool_name}({symbol})"
                                    )
                            except Exception as cache_error:
                                logger.warning(
                                    f"Failed to cache {tool_name}({symbol}): {cache_error}"
                                )
            ...
            if state.get("news_data"):
                for symbol, articles in state["news_data"].items():
                    try:
                        ...
                        logger.warning(f"Error streaming news for {symbol}: {e}")
            ...
            if additional_needed["needed"] and additional_needed.get("tools"):
                ...
                reasoning = additional_needed.get(
                    "reasoning", "Additional data needed for comprehensive analysis."
                )
                ...
                reasoning = additional_needed.get(
                    "reasoning",
                    "I have all the necessary data for comprehensive analysis.",
                )
            ...
            self.memory.save_message(session_id, "user", user_query, user_metadata)
            self.memory.save_message(
                session_id, "assistant", final_response, assistant_metadata
            )
    ...
    def _classify_query_type(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["compare", "versus", "vs", "better"]):
            ...
            word in query_lower
            for word in ["valuation", "worth", "value", "p/e", "pe ratio"]
        ):
            return "valuation"
        elif any(
            word in query_lower for word in ["earnings", "profit", "revenue", "income"]
        ):
            return "earnings"
        elif any(
            word in query_lower
            for word in ["cash", "liquidity", "balance sheet", "debt"]
        ):
            return "financial_health"
        elif any(
            word in query_lower
            for word in ["how is", "performance", "analysis", "overview"]
        ):
            return "performance"
        else:
            return "general"

    def _is_data_in_session_cache(
        self, session_id: str, symbol: str, tool_name: str
    ) -> bool:
        """Check if data exists in session cache."""
    ...
    def _add_to_session_cache(
        self, session_id: str, symbol: str, tool_name: str, data: Dict
    ):

...
class HybridMemoryManager:
    ...
    def _build_context_for_session(self, session_id: str) -> str:
        """
    ...
    def _get_recent_metadata(self, session_id: str, limit: int = 2) -> List[str]:
        ...
        try:
            ...
            for msg in reversed(result.data):  # Reverse to chronological order
                ...
                if symbols:
                    summaries.append(
                        f"User analyzed {', '.join(symbols)} ({query_type})"
                    )
    ...
    def generate_cumulative_context(
        self, session_id: str, llm_instance
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            response = llm_instance.invoke(
                [
                    SystemMessage(
                        content="You are a conversation summarizer. Be concise but capture key insights."
                    ),
            ...
            logger.info(
                f"HybridMemory: Generated context ({len(cumulative_context)} chars)"
            )
    ...
    def save_cumulative_context(self, session_id: str, cumulative_context: str):
        ...
        try:
            ...
            if result.data:
                ...
                logger.info(
                    f"HybridMemory: Saved cumulative context to message {message_id}"
                )
    ...
    def _build_initial_context_prompt(self, messages: List[Dict]) -> str:
        ...
        message_text = "\n".join(
            [f"- {msg['role']}: {msg['content'][:100]}" for msg in messages]
        )
...
Provide a concise summary:"""
    ...
    def _build_update_context_prompt(
        self, previous_context: str, new_messages: List[Dict]
    ) -> str:
        """Build prompt for updating existing context."""
        message_text = "\n".join(
            [f"- {msg['role']}: {msg['content'][:100]}" for msg in new_messages]
        )
...
Updated summary:"""
    ...
    def _update_conversation_title_if_new(self, conversation_id: str, first_query: str):
        ...
        try:
            ...
            if result.data and result.data.get("title") == "New Conversation":
                title = (
                    first_query[:57] + "..." if len(first_query) > 60 else first_query
                )

                self.supabase.table("conversations").update({"title": title}).eq(
                    "id", conversation_id
                ).execute()
        except Exception as e:

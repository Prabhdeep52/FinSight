...
def get_supabase_client() -> Client:
    ...
    if _supabase_client is None:
        ...
        if not settings.supabase_url or not settings.supabase_anon_key:
            logger.error(
                "get_supabase_client: Missing supabase_url or supabase_anon_key in settings"
            )
            raise ValueError(
                "Supabase URL and key must be set in settings/environment variables"
            )
        ...
        try:
            ...
            logger.error(
                f"get_supabase_client: Failed to initialize Supabase client: {str(e)}"
            )
            raise
...
class SupabaseManager:
    ...
    def __init__(self):
        ...
        if not self.url or not self.key:
            logger.error(
                "SupabaseManager: Missing supabase_url or supabase_anon_key in settings"
            )
            raise ValueError(
                "Supabase URL and key must be set in settings/environment variables"
            )
        ...
        try:
            ...
            logger.error(
                f"SupabaseManager: Failed to initialize Supabase client: {str(e)}"
            )
            raise
    ...
    def get_stock_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = (
                self.client.table("stock_overview")
                .select("*")
            ...
            if response.data and len(response.data) > 0:
                ...
                if last_updated_str:
                    ...
                    if age_hours < 24:  # Cache is fresh (less than 24 hours old)
                        logger.info(
                            f"SupabaseManager: Cache hit for {symbol}, age: {age_hours:.2f} hours"
                        )
                        return record.get("data")
                    else:
                        logger.info(
                            f"SupabaseManager: Cache expired for {symbol}, age: {age_hours:.2f} hours"
                        )
                        return None
                else:
                    logger.warning(
                        f"SupabaseManager: No timestamp found for {symbol}, treating as stale"
                    )
                    return None
            else:
    ...
    def save_stock_data(self, symbol: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.data:
                ...
                logger.error(
                    f"SupabaseManager: Failed to save data for {symbol} - no response data"
                )
                return False
    ...
    def _get_statement_record(
        ...
        try:
            ...
            logger.error(
                f"SupabaseManager: Error fetching record from {table} for {symbol}: {str(e)}"
            )
            return None

    def get_statement_data(
        self, table: str, symbol: str, max_age_hours: int = 24
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not record:
                logger.info(
                    f"SupabaseManager: No cached record in {table} for {symbol}"
                )
                return None
            ...
            if not last_updated_str:
                logger.warning(
                    f"SupabaseManager: No timestamp found in {table} for {symbol}, treating as stale"
                )
                return None
            ...
            if age_hours < max_age_hours:
                logger.info(
                    f"SupabaseManager: Cache hit in {table} for {symbol}, age: {age_hours:.2f} hours"
                )
                return record.get("data")
            else:
                logger.info(
                    f"SupabaseManager: Cache expired in {table} for {symbol}, age: {age_hours:.2f} hours"
                )
                return None

        except Exception as e:
            logger.error(
                f"SupabaseManager: Error getting statement data from {table} for {symbol}: {str(e)}"
            )
            return None

    def save_statement_data(
        self, table: str, symbol: str, data: Dict[str, Any]
    ) -> bool:
        ...
        try:
            logger.info(
                f"SupabaseManager: Saving record to table '{table}' for symbol: {symbol}"
            )
            upsert_data = {
                "symbol": symbol.upper(),
                "data": data,
            ...
            if response.data:
                logger.info(
                    f"SupabaseManager: Successfully saved statement for {symbol} to {table}"
                )
                return True
            logger.error(
                f"SupabaseManager: Failed to save statement for {symbol} to {table} - no response data"
            )
            return False

        except Exception as e:
            logger.error(
                f"SupabaseManager: Error saving statement to {table} for {symbol}: {str(e)}"
            )
            return False
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            fresh_records = (
                fresh_response.count
                if hasattr(fresh_response, "count")
                else len(fresh_response.data or [])
            )
            ...
            stats = {
                ...
                "cache_hit_rate": (fresh_records / total_records * 100)
                if total_records > 0
                else 0,
            }
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = (
                self.client.table("stock_overview").select("symbol").limit(1).execute()
            )

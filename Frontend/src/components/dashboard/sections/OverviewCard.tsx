...
export const OverviewCard: React.FC<OverviewCardProps> = ({
  ...
  const overview = stockData || {};

  const marketCap = overview.market_cap as string | undefined;
  const peRatio = overview.pe_ratio as string | undefined;
  const week52High = overview.high_52week as string | undefined;
  const week52Low = overview.low_52week as string | undefined;
  const beta = overview.beta as string | undefined;
  const dividendYield = overview.dividend_yield as string | undefined;
    ...
    <div className="bg-gradient-to-br from-blue-900/30 to-purple-900/30 rounded-lg border border-blue-500/30 p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          {companyName && <p className="text-gray-400 mt-1">{companyName}</p>}
          {sector && (

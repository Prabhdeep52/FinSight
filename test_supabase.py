...
async def test_caching_functionality():
    ...
    try:
        ...
        if cache_info1.get('cache_hit'):
            print("⚠️  Expected cache miss but got cache hit. Cache might not have been cleared.")
        ...
        if result1.get('symbol') == result2.get('symbol') and result1.get('name') == result2.get('name'):
...
async def test_cache_expiry():
    ...
    try:
        ...
        print("✅ Cache expiry logic is implemented in SupabaseManager.get_stock_data()")
...
async def test_error_handling():
    ...
    try:
        ...
        if result.get('error'):
            ...
            print("⚠️  Invalid symbol didn't return error (might be valid or rate limited)")

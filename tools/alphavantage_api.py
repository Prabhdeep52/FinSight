...
class AlphaVantageAPI:
    ...
    def fetch_earnings(self, symbol: str) -> Dict[str, Any]:
        ...
        return self._fetch_statement_with_cache(symbol, 'earnings', ALPHAVANTAGE_FUNCTIONS['earnings'], table='earnings')

    def fetch_income_statement(self, symbol: str) -> Dict[str, Any]:
        """Fetch income statement for a symbol (cache-first)."""
        return self._fetch_statement_with_cache(symbol, 'income_statement', ALPHAVANTAGE_FUNCTIONS['income_statement'], table='income_statements')

    def fetch_balance_sheet(self, symbol: str) -> Dict[str, Any]:
        """Fetch balance sheet for a symbol (cache-first)."""
        return self._fetch_statement_with_cache(symbol, 'balance_sheet', ALPHAVANTAGE_FUNCTIONS['balance_sheet'], table='balance_sheets')

    def fetch_cash_flow(self, symbol: str) -> Dict[str, Any]:
        """Fetch cash flow statement for a symbol (cache-first)."""
        return self._fetch_statement_with_cache(symbol, 'cash_flow', ALPHAVANTAGE_FUNCTIONS['cash_flow'], table='cash_flows')

    def _fetch_statement_with_cache(self, symbol: str, friendly_name: str, function_name: str, table: str) -> Dict[str, Any]:
        """Generic helper to fetch a statement using cache-first approach."""
        logger.info(f"AlphaVantageAPI: LLM requested {friendly_name} for symbol: {symbol}")
        ...
        if self.db:
            try:
                ...
                if cached:
                    logger.info(f"AlphaVantageAPI: Returning cached {friendly_name} from Supabase for {symbol}")
                    cached['cache_info'] = {'cache_hit': True, 'data_source': 'supabase_cache', 'table': table}
                    return cached
            except Exception as e:
                logger.error(f"AlphaVantageAPI: Error accessing Supabase cache for {symbol} ({friendly_name}): {str(e)}")

        # Cache miss - call Alpha Vantage
        logger.info(f"AlphaVantageAPI: Cache miss - fetching {friendly_name} from Alpha Vantage for {symbol}")
        try:
            params = {
            ...
            if response.status_code != 200:
                logger.error(f"AlphaVantageAPI: HTTP error {response.status_code} for {symbol} ({friendly_name})")
            ...
            if 'Note' in data or 'Information' in data:
                return {'error': True, 'message': 'API rate limit or information', 'details': data.get('Note') or data.get('Information')}

            result = {'symbol': symbol, 'data': data, 'data_source': f'alpha_vantage_{friendly_name}'}
            ...
            if self.db:
                def save():
                    try:
                        ...
                        if saved:
                            logger.info(f"AlphaVantageAPI: Cached {friendly_name} for {symbol} in {table}")
                        else:
                            logger.warning(f"AlphaVantageAPI: Failed to cache {friendly_name} for {symbol}")
                    except Exception as e:
                        logger.error(f"AlphaVantageAPI: Background cache save error for {symbol} ({friendly_name}): {str(e)}")

                t = threading.Thread(target=save, daemon=True)
                t.start()

            result['cache_info'] = {'cache_hit': False, 'data_source': 'alpha_vantage_api', 'table': table}
            return result

        except requests.RequestException as e:
            logger.error(f"AlphaVantageAPI: Network error fetching {friendly_name} for {symbol}: {str(e)}")
            return {'error': True, 'message': f'Request failed: {str(e)}'}
        except Exception as e:
            logger.error(f"AlphaVantageAPI: Unexpected error fetching {friendly_name} for {symbol}: {str(e)}")
            return {'error': True, 'message': f'Processing failed: {str(e)}'}

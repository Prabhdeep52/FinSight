    ...
    def _parse_market_cap(self, value: str) -> Optional[float]:
        ...
        try:
            ...
            if 'cr' in value or 'crore' in value:
                number = safe_float(value.replace('cr', '').replace('crore', '').strip())
                return number * 10000000 if number else None  # Convert crores to actual value
            elif 'l' in value or 'lakh' in value:
                number = safe_float(value.replace('l', '').replace('lakh', '').strip())
                return number * 100000 if number else None  # Convert lakhs to actual value
            else:

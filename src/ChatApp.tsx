...
export default function ChatApp() {
  ...
  const handleSelectChat = async (selectedSessionId: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/v1/agent/chat-messages/${selectedSessionId}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
    ...
    <div className="flex flex-col h-screen bg-background text-foreground">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden">
          <DashboardLayout stocks={dashboardStocks} streamEvents={streamEvents} isStreaming={isStreaming} />
        </div>

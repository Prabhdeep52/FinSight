    ...
    return (
      <div className="flex-1 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-semibold mb-2">Welcome to Finance Bro</h2>
          <p className="text-muted-foreground mb-8">Ask me anything about stocks, companies, or financial markets</p>
              ...
              >
                {message.role === "user" ? (
                  ...
                  <div className="space-y-4">
                    {message.symbols && message.symbols.length > 0 && message.stockData && (
                      <div className="space-y-3">
                        {message.symbols.map((symbol) =>
                          message.stockData?.[symbol] ? (
                            <StockCard key={symbol} symbol={symbol} data={message.stockData[symbol]} />
                          ) : null,
                        )}
                      </div>
                    )}

                    {message.symbols && message.symbols.length > 0 && message.statementData && (
                      <div className="space-y-4">
                        {message.symbols.map((symbol) =>
                          message.statementData?.[symbol] ? (
                            <FinancialTables key={symbol} symbol={symbol} data={message.statementData[symbol]} />
                          ) : null,
                        )}
                      </div>
                    )}
                    ...
                    >
                      <ReactMarkdown
                        components={{
                          ...
                          pre: ({ children }) => (
                            <pre className="bg-muted p-3 rounded overflow-x-auto text-xs">{children}</pre>
                          ),
                        }}
                      >
                        {message.content}

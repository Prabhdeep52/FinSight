...
class OptimizedFinancialAgent:
    ...
    def __init__(self):
        ...
        logger.info(f"OptimizedFinancialAgent: LLM configured with model {self.settings.gemini_model}")
    ...
    def process_query(self, user_query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("OptimizedFinancialAgent: PHASE 1 - Creating tool execution plan")
            ...
            logger.info("OptimizedFinancialAgent: PHASE 2 - Executing tools in parallel")
            ...
            if additional_needed['needed']:
                logger.info(f"OptimizedFinancialAgent: Fetching additional data: {additional_needed['tools']}")
...
"""
        ...
        try:
            ...
            if json_match:
                ...
                logger.info(f"OptimizedFinancialAgent: Extracted symbols: {state['extracted_symbols']}")
            else:
...
"""
        ...
        try:
            ...
            response = self.llm.invoke([
                SystemMessage(content="You are a financial data analyst creating efficient data fetching plans."),
            ...
            if json_match:
                ...
                logger.info(f"LLM Plan: {plan.get('query_type', 'unknown')} - {plan.get('reasoning', 'no reasoning')[:100]}")
                return plan
            else:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    self._store_fetch_result(state, task['data_type'], task['symbol'], result)
                    
                    # Add to summary
                    if not result.get('error'):
                        state['fetch_summary'].append(f"✓ {task['data_type']} for {task['symbol']}")
                    else:
                        state['fetch_summary'].append(f"✗ {task['data_type']} for {task['symbol']}: {result.get('message', 'error')}")
                
                except Exception as e:
                    logger.error(f"Error fetching {task['data_type']} for {task['symbol']}: {e}")
                    state['fetch_summary'].append(f"✗ {task['data_type']} for {task['symbol']}: {str(e)}")
        
        logger.info(f"Parallel fetch completed. Summary: {len(state['fetch_summary'])} operations")
    ...
    def _store_fetch_result(self, state: AgentState, data_type: str, symbol: str, result: Dict[str, Any]):
...
"""
        ...
        try:
            response = self.llm.invoke([
                SystemMessage(content="You are reviewing if you have sufficient data to answer a financial query."),
            ...
            if json_match:
                ...
                logger.info(f"LLM Review: needed={review.get('needed', False)}, confidence={review.get('confidence', 'unknown')}")
                return review
            else:
                return {"needed": False, "reasoning": "Sufficient data available", "confidence": "medium"}
        
        except Exception as e:
            logger.error(f"Error in review: {e}")
            return {"needed": False, "reasoning": "Error in review, proceeding with available data", "confidence": "low"}
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if not result.get('error'):
                        state['fetch_summary'].append(f"Additional fetch: {task['data_type']} for {task['symbol']}")
                        logger.info(f"Successfully fetched additional {task['data_type']} for {task['symbol']}")
                    else:
                        logger.warning(f"Failed to fetch additional {task['data_type']} for {task['symbol']}: {result.get('message')}")
                
                except Exception as e:
                    logger.error(f"Error fetching additional {task['data_type']} for {task['symbol']}: {e}")
        
        logger.info(f"Additional data fetch completed. Total operations: {len(fetch_tasks)}")
    ...
    def _generate_response_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if stock_data_count == 0:
                state['final_response'] = "I couldn't retrieve sufficient data to analyze your query. Please try again."
                return state
            ...
            try:
                ...
                state['final_response'] = "Analysis is taking longer than expected. Please try again."
    ...
    def stream_execution(self, user_query: str):
        ...
        try:
            ...
            total_tools = sum(len(sp.get('data_needed', [])) for sp in tool_plan.get('symbols', []))
            ...
            if additional_needed.get('needed'):
                yield {
                    ...
                    'reasoning': additional_needed.get('reasoning', 'After reviewing what I have, I realize I need a bit more data to provide a complete analysis'),
                    'progress': 85
                }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if result.get('error'):
                        ...
                        insight = self._extract_insight(task['data_type'], result, task['symbol'])
    ...
    def _extract_insight(self, data_type: str, result: Dict[str, Any], symbol: str) -> str:
        ...
        try:
            ...
            return f'I have fetched the {data_type.replace("_", " ")} data for {symbol}.'
    
    
    def _extract_data_insight(self, data_type: str, symbol: str, result: Dict[str, Any]) -> str:

...
class FinancialLangGraphAgent:
    ...
    def __init__(self):
        ...
        logger.info(f"FinancialLangGraphAgent: LLM configured with model {self.settings.gemini_model}")
    ...
    def _parse_query_node(self, state: AgentState) -> AgentState:
        ...
        logger.info(f"FinancialLangGraphAgent: Processing user query: {state['user_query']}")
...
Example response:
{
  ...
  "needs_data": true
            ...
            try:
                ...
                logger.info(f"FinancialLangGraphAgent: Parsed response: {parsed_response}")
                
                state['extracted_symbols'] = parsed_response.get("symbols", [])
                state['current_step'] = "query_parsed"
                
                logger.info(f"FinancialLangGraphAgent: Extracted symbols: {state['extracted_symbols']}")
                
            except json.JSONDecodeError:
                logger.warning("FinancialLangGraphAgent: Failed to parse JSON, using fallback symbol extraction")
                # Fallback: simple keyword extraction
                query_upper = state['user_query'].upper()
                common_symbols = ["AAPL", "GOOGL", "MSFT", "AMZN", "TSLA", "META", "NVDA",
                                "INFY", "TCS", "RELIANCE", "WIPRO", "HDFC"]
                
                found_symbols = [symbol for symbol in common_symbols if symbol in query_upper]
                state['extracted_symbols'] = found_symbols
                logger.info(f"FinancialLangGraphAgent: Fallback extraction found: {found_symbols}")
            
            return state
            
        except Exception as e:
            logger.error(f"FinancialLangGraphAgent: Error in parse_query_node: {str(e)}")
    ...
    def _execute_tools_node(self, state: AgentState) -> AgentState:
        ...
        try:
            if not state['extracted_symbols']:
                logger.warning("FinancialLangGraphAgent: No symbols extracted, asking LLM to proceed")
            ...
            for symbol in state['extracted_symbols']:
                logger.info(f"FinancialLangGraphAgent: Fetching data for symbol: {symbol}")
                
                try:
                    # Use the stock tool to fetch data
                    stock_data_result = self.stock_tool._run(symbol)
                    logger.info(f"FinancialLangGraphAgent: Stock data fetched for {symbol}")
                    ...
                    if not stock_data.get('error'):
                        logger.info(f"FinancialLangGraphAgent: Analyzing data for {symbol}")
                        analysis_result = self.analysis_tool._run(stock_data_result)
                        analysis_data = json.loads(analysis_result)
                        state['analysis_results'][symbol] = analysis_data
                        logger.info(f"FinancialLangGraphAgent: Analysis completed for {symbol}")
                    else:
                        logger.warning(f"FinancialLangGraphAgent: Stock data contains error for {symbol}")
                        
                except Exception as e:
                    logger.error(f"FinancialLangGraphAgent: Error processing {symbol}: {str(e)}")
            ...
            logger.error(f"FinancialLangGraphAgent: Error in execute_tools_node: {str(e)}")
...
Think step by step about what information you need and which tools to use."""
            ...
            logger.info("FinancialLangGraphAgent: Received LLM response with potential tool calls")
            
            # Execute any tool calls    
            if response.tool_calls:
                logger.info(f"FinancialLangGraphAgent: LLM requested {len(response.tool_calls)} tool calls")
                
                for tool_call in response.tool_calls:
                    tool_name = tool_call["name"]
                    tool_args = tool_call["args"]
                    
                    logger.info(f"FinancialLangGraphAgent: Executing tool: {tool_name} with args: {tool_args}")
                    ...
                    if tool_name == "get_stock_data":
                        ...
                        if symbol:
                            ...
                            logger.info(f"FinancialLangGraphAgent: Fetched data for {symbol}")
                        ...
                        if stock_data:
                            ...
                            state['analysis_results'][symbol] = analysis_data
                            logger.info(f"FinancialLangGraphAgent: Analyzed data for {symbol}")
            
            state['current_step'] = "llm_tools_executed"
            return state
            
        except Exception as e:
            logger.error(f"FinancialLangGraphAgent: Error in LLM tool decision: {str(e)}")
    ...
    def _generate_response_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            logger.info("FinancialLangGraphAgent: Preparing response generation context")
            logger.debug(f"FinancialLangGraphAgent: Context contains {len(state['stock_data'])} stock data entries and {len(state['analysis_results'])} analysis results")
...
"""
            ...
            messages = [
                SystemMessage(content=system_prompt),
                HumanMessage(content=f"Please provide a comprehensive response based on this context:\n\n{context_str}")
            ]
    ...
    def _handle_error_node(self, state: AgentState) -> AgentState:
        ...
        logger.error(f"FinancialLangGraphAgent: Error encountered: {state['error_message']}")
        
        state['final_response'] = f"""I apologize, but I encountered an error while processing your request: {state['error_message']}
...
"""
    ...
    def process_query(self, user_query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"FinancialLangGraphAgent: Graph execution completed with step: {final_state.get('current_step', 'unknown')}")
            ...
            result = {
                ...
                "status": "success" if final_state.get('current_step') != "error" else "error",
                "error_message": final_state.get('error_message') if final_state.get('error_message') else None
            }
            
            logger.info("FinancialLangGraphAgent: Query processing completed successfully")
            return result
            
        except Exception as e:
            logger.error(f"FinancialLangGraphAgent: Unexpected error during query processing: {str(e)}")
            return {
                "query": user_query,

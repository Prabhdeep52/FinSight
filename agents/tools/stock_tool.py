...
class StockDataTool(BaseTool):
    ...
    def _run(self, symbol: str) -> str:
        ...
        try:
            ...
            logger.info(f"StockDataTool: Using direct FinancialAgent call for {clean_symbol}")
            ...
            logger.debug(f"StockDataTool: Response data keys: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
...
def create_stock_data_tool(api_base_url: str = "http://localhost:8000") -> StockDataTool:
    """
    Factory function to create a stock data tool.
    
    Args:
        api_base_url: Not used anymore - kept for backward compatibility
